...
try:
    ...
    def append_com():
        ...
        def select_folder():
            ...
            if folder_path:
                ...
                try:
                        # добавление кнопки сохранения команды, если записан голос
                    if speech:
                        append = tk.Button(append_window, text="Добавить команду", command=add, font=('Fira Sans Regular', 10))
                        append.pack(pady=30)
                except:
    ...
    def save_num():
        ...
        if 1 <= int(inp) <= x:
            ...
            err.config(text='Нет такого номера!')
    ...
    def del_com():
        ...
        err = tk.Label(del_window, text="", font=('Fira Sans Regular', 14), bg='#bec3c6')
        err.pack(pady=10)
    ...
    def read_file(window):
        with open(f'{dir_path}\\commands.txt', 'r', encoding='utf-8') as file:
            ...
            lines = file.readlines()
            x = len(lines)
            # цикл на максимум 10 элементов, который отрисовывает сохранённые команды
            for i in range(10):
                try:
